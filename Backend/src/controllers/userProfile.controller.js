...
//Book Doctor (coupon management)
    ...
    const BookAppointment = asyncHandler(async (req, res) => {
      ...
      const { queue, selectedTime } = req.body;
      ...
      const couponNumber = appointmentsToken[selectedTime];

      if (!couponNumber) {
        throw new ApiError(400, "Invalid time slot selected.");
      }

      // Check if this token is already booked for this queue
      const existingAppointment = await Appointment.findOne({
        partOfQueue: queue._id,
        couponNumber: couponNumber,
        status: { $ne: "Cancelled" } // Allow re-booking only if cancelled
      });
