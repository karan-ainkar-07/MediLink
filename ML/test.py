...
def predict_topK(symptoms, k):
    ...
    if isinstance(symptoms, list):
        ...
        if len(symptoms) > 0 and isinstance(symptoms[0], str):
            user_input = {s: 1 for s in symptoms}

        # case 3: list of objects
        elif len(symptoms) > 0 and isinstance(symptoms[0], dict):
            ...
            raise ValueError("Invalid list format for symptoms")

    elif isinstance(symptoms, dict):
        # case 2
        for s, w in symptoms.items():
            w = max(0, min(1, float(w)))
            user_input[s] = w
